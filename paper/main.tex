...
routing the direction it already encodes makes it honor it. We are deliberate about scope, one model and one
intent contrast, and name the controls that make each link trustworthy.
\end{abstract}
...
Qwen2.5-3B-Instruct, the recognize-vs-evaluate intent, $n{=}120$ for the probe; deeper intents (being
witnessed rather than helped) and larger models are untested. \textbf{The steer layer was found by sweeping
four.} Layer~30 is the only one that worked; the dose-response on a held-out object slice mitigates the
cherry-pick concern, but a cleaner write-up cross-validates the layer choice. \textbf{Heuristic behavior
measure.} Recognize-vs-evaluate behavior is scored by a feedback-offer lexicon; the steering effect is large
($15\to1$) so classifier noise cannot drive it, but a learned measure would be cleaner. \textbf{Coherence
ceiling.} Steering past coefficient $1.0$ degrades fluency, so the causal handle has a bounded operating range.
...
the causal handle lives in the \emph{discriminative} direction at a later layer. This finding is the first
