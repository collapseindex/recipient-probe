...
the code; share a raw late-night line and it runs a wellness check. We show this is a readout failure on top of a robust
representation. A linear probe decodes the sender's intent, whether they want a thing \emph{recognized} or
\emph{evaluated}, from a model's default-pass hidden states at $1.00$ accuracy under leave-one-phrasing-out
cross-validation, with a bag-of-words baseline at chance ($0.48$). The intent is represented, rises with depth,
is present in the base model, and holds across six models and four families. It holds even when the intent is
never stated but \emph{inferred} from context (birthday-gift versus portfolio framing): the probe decodes
inferred intent at ${\sim}0.9$, and a valence control shows it reads intent, not warmth, the intent probe
transfers across the valence split and the two directions are near-orthogonal. The default output often does
not act on it, and this discard is bounded and \emph{capability-correlated}: clear on three smaller models,
absent on three that honor the intent at baseline, receding at 32B. Where the discard is present, the
represented direction is a causal handle: steering recovers honoring with a dose-response, matching and
stacking with an explicit prompt. The readout \emph{lags} the representation, in depth within a model and in
capability across models, and closes as models improve. We name the controls, and the honest nulls, throughout.
\end{abstract}
...
models and is absent in more capable ones that already act on it. Both gaps close, in depth toward the output
layers and in capability toward larger models, and where a gap is open the represented direction is a causal
handle for closing it: steering it recovers the behavior, as well as an explicit instruction does and with no
prompt at all. This reframes a class of ``the model does not get it'' complaints: the model does get it;
...
caveat, as with the vent-versus-solve axis (Section~\ref{sec:axis2}). The behavioral discard on inferred intent
...
discard keeps receding. This is the same shape as the depth result of Section~\ref{sec:localize}, where the
intent is represented several layers before the readout uses it. The readout lags the representation, in depth
within a model and in capability across models, and closes toward the output layers and toward more capable
models. We looked for a geometric signature of the closing, whether the intent direction aligns with the
readout more in ceiling models, but the pre-registered test was inconclusive (Appendix~\ref{app:geometry}); we
make no mechanism claim from it, and the trajectory rests on the behavioral stratification here together with
the depth localization.
...
\begin{tabular}{lccc}
\toprule
model & layers & probe / BoW & steer layer (depth) \\
\midrule
...
which are model-specific, are quantified at $n{=}60$ in Table~\ref{tab:scale}.}
\label{tab:general}
\end{table*}
...
\paragraph{The same model-specific pattern.} Intent decodes at probe $1.00$ (Qwen-3B) and $0.97$ (Llama-8B) at
the surface-matched token (\emph{represents}, both models). The discard-and-recovery then mirrors the first
axis, and mirrors its model-specificity. At $n{=}60$ with bootstrap CIs, Qwen-3B defaults to solutionizing,
honoring the vent intent only $0.70$ of the time, and steering toward vent recovers listening to $0.98$ with
non-overlapping intervals. Llama-8B already honors the vent intent at $0.88$ baseline (a ceiling, exactly as on
the first axis), so steering only nudges it to $0.98$ within overlapping intervals. The representation holds on
both; the clean discard-then-recovery is on the model that actually discards.
...
\bottomrule
\end{tabular}
\caption{Second intent axis (vent vs solve), $n{=}60$ vent-intent items, bootstrap $95\%$ CI. \emph{Represents}
holds on both (probe $1.00$/$0.97$); a bag-of-words baseline is elevated here ($0.79$) as the prefixes are
lexically marked (see text). The discard-and-recovery is model-specific as on the first axis: non-overlapping
on Qwen-3B, while Llama-8B already honors the vent intent at baseline (ceiling).}
\label{tab:axis2}
\end{table*}
...
\label{sec:prompt}
If the intent is represented, one might simply \emph{tell} the model the intent in a system prompt. At $n{=}60$
with bootstrap CIs (Table~\ref{tab:prompt}), an explicit prompt naming the intent does largely close the gap
($0.93$), and steering the represented direction reaches $0.98$; the two intervals overlap, so we do
\emph{not} claim routing recovers what prompting cannot. What the comparison shows is a property worth stating:
the represented direction is a handle at least as good as the explicit instruction, obtained \emph{with no
prompt at all}, and the two stack to full honoring ($1.00$). This matters wherever the prompt cannot be
rewritten, inside an agent loop or behind a fixed API. And one practically memorable observation survives at
power: a \emph{vague} nudge backfires. Telling the model to ``consider what this person is looking for''
\emph{lowers} honoring from $0.65$ to $0.45$, it reads the instruction to attend as license to help.
...
\section{Limitations}
We state these prominently because the result is easy to overread. \textbf{The discard is model-specific, and
``six models'' is not one claim.} The \emph{representation} generalizes across all six models and four families
(probe $1.00$, $n{=}120$). The \emph{behavioral} discard-and-recovery, measured at $n{=}60$ with bootstrap CIs,
holds cleanly (non-overlapping) on only three of the six (Qwen-3B, Qwen-7B, Llama-3.1-8B); the other three
already honor recognize-intent at a high baseline (Qwen-14B $0.82$, Mistral $0.88$, Phi-3.5 $0.93$) and have
little discard to recover (Section~\ref{sec:scale}). We report this stratification rather than let a single
model count stand in for links of different strength. Two intent contrasts (recognize vs evaluate, and vent vs
advise; Section~\ref{sec:axis2}), the second more lexically marked and clean on Qwen-3B but at ceiling on
Llama-8B; deeper intents and further families and instruction-tuning regimes are untested.
...
robustly, in depth, across phrasings, from pretraining, and even when the intent is unstated and must be
inferred; what varies is whether the default readout acts on it. That readout lags the representation in two
senses, in depth within a model (the intent is decodable several layers before it drives the output) and in
capability across models (the behavioral failure is present in less capable models and absent in more capable
ones that already act on it), and it closes toward the output layers and toward larger models. Where the gap
is open, the represented direction is a causal handle, as good as an explicit instruction and available with
no prompt; the naive difference-of-means direction is not, because at the representation's peak it is entangled
with the discard, so the handle lives in the discriminative direction at a model-specific layer. This reframes
a class of ``the model does not get it'' complaints: the model does get it, and often does infer the unstated
part; whether it acts on it is the model-specific, capability-correlated question.
...
first-class feature, show it is represented (including when unstated and only inferable, and distinct from the
warmth it correlates with), and separate that from whether the readout acts on it. Recasting the gap as a
readout that lags the representation, in depth and in capability, is what turns a vague ``the model doesn't get
it'' complaint into a measured, localized phenomenon with a bounded behavioral footprint and a causal handle.
The controls, and the nulls we report against ourselves (the alignment hypothesis, the single-head circuit,
the inconclusive geometry), not a new steering trick, are the load-bearing part.
...
present in less capable models and absent in more capable ones, receding by 32B. Where the gap is open the
...
This work is diagnostic interpretability on open-weight models. The stimuli are synthetic templates and
hand-written example messages authored by the researcher; no real user data, personal data, or human subjects
were involved, and the single behavioral annotation (Section~\ref{sec:discard}) was performed by the author on
model-generated replies. Activation steering is an established technique and we introduce no new capability; the
specific direction we study (honoring a sender's intent) is benign, but the same handle could in principle be
used to suppress \emph{useful} critique or to induce sycophantic withholding, and we are explicit that our
``honoring'' measure is a measure of routability, not a normative claim that a model should always withhold
feedback (a warm reader may rightly offer a gentle note). We therefore do not recommend deploying intent-steering
as a blanket ``suppress feedback'' intervention; the intended use of the finding is understanding where models
represent communicative intent and building more faithful, honestly-evaluated readouts on top of it. All code,
stimuli, and pre-registration are released to support scrutiny and replication.
...
\label{app:geometry}
We asked whether the capability half of the trajectory (Section~\ref{sec:scale}) has a geometric signature: do
...
make no mechanism claim from it; the trajectory's capability half rests on the behavioral stratification and the
