...
behavior with a monotone dose-response. The representation holds across six models and four families
(probe $1.00$); the readout discard, though, is model-specific: on three of the six the discard is real and
the recovery is non-overlapping at $n{=}60$, while the other three already honor the intent at baseline. The steering handle is specific to the learned direction, norm-matched random
directions do not reproduce it, and the pattern recurs on a second intent axis. The discard localizes in depth: the
